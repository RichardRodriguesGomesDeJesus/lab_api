...
function createUser(req: Request, res: Response) {
    ...
    const user = userRepository.createUser(data.username, data.password, 
        data.userType, new Date().toISOString().split("T")[0], data.active);
...
function updateUser(req: Request, res: Response) {
    ...
    const user = userRepository.updateUserById(updatedUser.username, updatedUser.password,
        updatedUser.userType, updatedUser.active, id)
